...
import com.mobilelife.persistance.entities.OrderPlanprimary;
...
public class OrderPlanprimaryDao {
	...
	public int findId()
	{
		...
		int rid = 0;
		try {
			List<OrderPlanprimary> orderPlanprimaryList = null;
			...
			orderPlanprimaryList = new HibernateDAO().findBySQLQuery(session, OrderPlanprimary.class, query,"order_planprimary");
	...
	public void saveData(OrderPlanprimary entityObj) {
		Session session = HibernateSessionManager.getSession();
		Transaction tx = null;
		try {
	...
	public OrderPlanprimary vaidateOrderidCouponid(String couponid) {
		OrderPlanprimary orderPlanprimary = null;
