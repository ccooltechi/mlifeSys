...
import com.mobilelife.persistance.entities.SearchDetailsEntity;
...
public class SearchDetailsDao {
	...
	public List<SearchDetailsEntity> findAll()
	{
		Session session = TelcoMisHibernateSessionManager.getSession();
		List<SearchDetailsEntity> userInfoList = null;
		Query query = session.getNamedQuery("SearchDetails.findAll");
		System.out.println("Size = "+query.list().size());
		userInfoList = (List<SearchDetailsEntity>)query.list();
		return userInfoList;
	}

	public List<SearchDetailsEntity> findSearchInfoByID(String ssoUserId)
	{
		Session session = TelcoMisHibernateSessionManager.getSession();
		List<SearchDetailsEntity> userInfoList = null;
	...
	public int findId()
	{
		...
		int rid = 0;
		try {
			List<SearchDetailsEntity> resultList = null;

			String query = "Select * from search_details order by id desc limit 2";
			logger.debug("query in findID in  search_details " + query);
			resultList = new HibernateDAO().findBySQLQuery(session, SearchDetailsEntity.class, query,"search_details");
	...
	public void saveData(SearchDetailsEntity entityObj) {
		Session session = TelcoMisHibernateSessionManager.getSession();
		Transaction tx = null;
		try {
